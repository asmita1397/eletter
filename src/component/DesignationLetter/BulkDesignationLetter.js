//This is bulk uploading Designation letter file. 
//In this file by uploading bulk details with excel format, 
//The user will get all Designation letter in the document format with letter header and footer.
...
export class BulkDesignationLetter extends Component {
        ...
        if (this.state.employee.length > 0) {
            return (
                this.state.employee.map((item,index) => {
                    ...
                    effectDate = new Date((item.effdate - (25567 + 2)) * 86400 * 1000)
                    return (
                        <div>
                                            ...
                                            <div className="card-body pb-0">
                                                ...
                                                <div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <p style={{ textAlign: 'justify', paddingLeft: 20, paddingRight: 20, margin: 0 }}><strong> {item.employeeName}</strong>,</p>
                                                        <p style={{ textAlign: 'justify', paddingLeft: 20, paddingRight: 20, margin: 0 }}> {item.employeeId}</p>
                                                        <p style={{ textAlign: 'justify', paddingLeft: 20, paddingRight: 20 }}>&nbsp;</p>
                                                        <p style={{ textAlign: 'justify', paddingLeft: 20, paddingRight: 20 }}>Dear <strong> {item.employeeName}</strong>,</p>
                                                        <br />
                                                        <p style={{ textAlign: 'justify', paddingLeft: 20, paddingRight: 20 }}>We are pleased to inform your designation has been changed as <strong> {item.newDesignation} </strong>effective <strong>{effectDate.getDate()}<sup>{this.nth(effectDate.getDate())}</sup>&nbsp;{moment(effectDate).format('MMMM YYYY')}</strong>. Other terms and conditions remain the same as per the appointment letter.</p>
...
export default withRouter(BulkDesignationLetter)