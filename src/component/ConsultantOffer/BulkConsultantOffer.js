//This is bulk uploading Consultant letter file. 
//In this file by uploading bulk details with excel format, 
//The user will get all Consultant letter in the document format with letter header and footer.
...
export class BulkConsultantOffer extends Component {
    ...
    if (this.state.employee.length > 0) {
      return (
        this.state.employee.map((item,index) => {
          ...
          reportingDate = new Date((item.reportingDate - (25567 + 2)) * 86400 * 1000)
                ...
                <PdfContainer id={index} name={item.employeeName} createPdf={this.createPdf}>
                  <div className="card" id="AFourPage">
                    ...
                    <div>
                      ...
                      <div>
                        <p style={{ textAlign: 'left', paddingLeft: 30, paddingRight: 30, marginTop: 50 }} align="JUSTIFY"><span ><strong>{toDayDate.getDate()}<sup>{this.nth(toDayDate.getDate())}</sup>&nbsp;{moment(toDayDate).format('MMMM YYYY')}</strong></span></p>

                        <p style={{ textAlign: 'justify', paddingLeft: 30 }}>&nbsp;</p>
                        <p style={{ textAlign: 'justify', paddingLeft: 30 }}><strong>Dear {item.salute}{item.employeeName},</strong></p>
                        <p style={{ textAlign: 'justify', paddingLeft: 30 }}>&nbsp;</p>
                        <p style={{ textAlign: 'justify', paddingLeft: 30 }}>We are pleased to offer you an employment at Test Yantra Software Solutions (India) Pvt. Ltd. You have been offered as a <strong>Consultant - {item.designation}</strong> in our Bangalore office. We extend this offer, and the opportunity it represents, with great confidence in your abilities.&nbsp;</p>
                        <p style={{ textAlign: 'justify', paddingLeft: 30 }}>Your date of joining to Test Yantra would be <span ><strong>{joiningDate.getDate()}<sup>{this.nth(joiningDate.getDate())}</sup>&nbsp;{moment(item.joiningDate).format('MMMM YYYY')}</strong></span>. Your Initial Posting will be at<strong> {item.companyLocation}. </strong>However, your services are transferable to any other place or office of the Company or to any subsidiary or associate company; whether now existing or still to be formed. Such transfer / deputation will be in accordance with the company’s rules being in force at the time.</p>
                  ...
                  <div className="card-body pb-0">
                    ...
                    <div>
                      ...
                      <div>
                        ...
                        <table style={{ paddingLeft: 30, width: 600, marginLeft: 30, }}  >
                          <tbody style={{ paddingLeft: 30 }}>
                            <tr className="trStyle" style={{ paddingLeft: 39, boder: "1px solid black" }} >
                              <td className="trStyle" style={{ paddingLeft: 16, width: 325, boder: "1px solid black" }}>
                                <p><strong>Salary Compensation on Bench </strong></p>
                              </td>
                              <td className="trStyle" style={{ paddingLeft: 29, width: 207 }}>
                                <p style={{ paddingLeft: 30 }}><strong>8,000 per month </strong></p>
                              </td>
                            </tr>
                            <tr className="trStyle" style={{ paddingLeft: 30, border: "1px solid black" }} >
                              <td className="trStyle" style={{ paddingLeft: 30, width: 234 }}>
                                <p style={{ paddingLeft: 30 }}><strong>&nbsp;</strong></p>
                              </td>
                              <td className="trStyle" style={{ paddingLeft: 30, width: 207 }}>
                                <p style={{ paddingLeft: 30 }}><strong>&nbsp;</strong></p>
                              </td>
                            </tr>
                          </tbody>
                        ...
                        <p style={{ textAlign: 'justify', paddingLeft: 30 }}>&nbsp;</p>
                        <p style={{ textAlign: 'justify', paddingLeft: 30 }}>I agree to accept the conditional offer and employment on the above-mentioned terms and conditions. I’ll report for duty on <span ><strong>{reportingDate.getDate()}<sup>{this.nth(reportingDate.getDate())}</sup>&nbsp;{moment(item.reportingDate).format('MMMM YYYY')}</strong></span></p>
...
export default withRouter(BulkConsultantOffer)
