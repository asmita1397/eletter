//This is Input BDPolicy file. It contains all required details about BDPolicy file.
...
export class InputBDPolicy extends Component {
    ...
    validation=() => {
        ...
        if (!address.match(/^[A-Za-z\s]+$/) || address === "" || address.length > 100) {
            if (address === "") {
                ...
                this.setState({
                    showAddress: true,
                    errorMsgAddress: "Address field length should less than 30"
                })
            }
            else {
                this.setState({
        ...
        if (!age.match(/^[0-9]+$/) || age === "" || age.length > 3 || Number(age) === 0) {
            if (age === "") {
                ...
                this.setState({
                    showAge: true,
                    errorMsgAge: "Age must be greater than zero"
                })
            }
    ...
    handleChildData = (data) => {
        ...
        if (data !== "") {
            this.props.history.push('/bulkbdpolicy')
        }
    }
    render() {
